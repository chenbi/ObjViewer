...
public class GeomBuilder
  /*
    ...
    to define points, and objviewer.GeomBuilder.Color to define colours), and
    use the returned vertex indices to construct faces with AddTri and
    AddQuad. Finally, call MakeObj to obtain a objviewer.GeomBuilder.Obj that
  ...
  */ {
    ...
    public GeomBuilder (boolean GotNormals, /* vertices will have normals specified, otherwise they will be automatically generated for flat shading */
        ...
        BoundMin=null;
        BoundMax=null;
    } /*objviewer.GeomBuilder*/

    private int Add (Vec3f Vertex,
      /* following args are either mandatory or must be null, depending
        on respective flags passed to constructor */
                     Vec3f Normal, Vec3f TexCoord, Color VertexColor, boolean AutoNormals)
      /* adds a new vertex, and returns its index for use in constructing faces. */ {
        if (AutoNormals != (Normal == null) || (PointColors == null) != (VertexColor == null) || (PointTexCoords == null) != (TexCoord == null)) {
        ...
        if (AutoNormals == this.AutoNormals) {
            if (BoundMin != null) {
                BoundMin=new Vec3f (Math.min (BoundMin.x, Vertex.x), Math.min (BoundMin.y, Vertex.y), Math.min (BoundMin.z, Vertex.z));
            } else {
                BoundMin=Vertex;
            } /*if*/
            if (BoundMax != null) {
                BoundMax=new Vec3f (Math.max (BoundMax.x, Vertex.x), Math.max (BoundMax.y, Vertex.y), Math.max (BoundMax.z, Vertex.z));
            } else {
                BoundMax=Vertex;
            } /*if*/
        } /*if*/
    ...
    public int Add (Vec3f Vertex,
      /* following args are either mandatory or must be null, depending
        on respective flags passed to constructor */
                    Vec3f Normal, Vec3f TexCoord, Color VertexColor)
      /* adds a new vertex, and returns its index for use in constructing faces. */ {
        return Add (
                /*Vertex =*/ Vertex,
                /*Normal =*/ Normal,
    ...
    private int AddActual (int VertIndex, Vec3f Normal) {
        if (!AutoNormals) {
            throw new RuntimeException ("objviewer.GeomBuilder.AddActual shouldn’t be called if not AutoNormals");
        } /*if*/
        return Add (
    ...
    private void GenAutoNormal (int[] Vertices
          /* assume length at least 3 and coplanar if > 3, replaced with final generated vertices */) {
        if (!AutoNormals) {
            throw new RuntimeException ("objviewer.GeomBuilder.GenAutoNormal shouldn’t be called if not AutoNormals");
        } /*if*/
        final Vec3f V1=TempPoints.get (Vertices[0]), V2=TempPoints.get (Vertices[1]), V3=TempPoints.get (Vertices[2]);
    ...
    private void AddTri (int V1, int V2, int V3, boolean AutoNormals)
    ...
    public Obj MakeObj ()
      /* constructs and returns the final geometry ready for rendering. */ {
        if (Points.size () == 0) {
            throw new RuntimeException ("objviewer.GeomBuilder: empty object");
        } /*if*/
      ...
      /* Need to use allocateDirect to allocate buffers so garbage
        ...
        VertexBuffer=ByteBuffer.allocateDirect (Vertices.length * 4).order (ByteOrder.nativeOrder ()).asIntBuffer ().put (Vertices);
        VertexBuffer.position (0);
        if (PointNormals != null) {
            NormalBuffer=ByteBuffer.allocateDirect (Normals.length * 4).order (ByteOrder.nativeOrder ()).asIntBuffer ().put (Normals);
            NormalBuffer.position (0);
        } else {
            NormalBuffer=null;
        } /*if*/
        if (PointTexCoords != null) {
            TexCoordBuffer=ByteBuffer.allocateDirect (TexCoords.length * 4).order (ByteOrder.nativeOrder ()).asIntBuffer ().put (TexCoords);
            TexCoordBuffer.position (0);
        } else {
            TexCoordBuffer=null;
        } /*if*/
        if (PointColors != null) {
            ColorBuffer=ByteBuffer.allocateDirect (Colors.length * 4).order (ByteOrder.nativeOrder ()).asIntBuffer ().put (Colors);
        ...
        return new Obj (VertexBuffer, NormalBuffer, TexCoordBuffer, ColorBuffer, IndexBuffer, NrIndexes, BoundMin, BoundMax);
    } /*MakeObj*/
    ...
    public static class Obj
      /* representation of complete object geometry. */ {
        ...
        private Obj (IntBuffer VertexBuffer, IntBuffer NormalBuffer, /* optional */
                     IntBuffer TexCoordBuffer, /* optional */
                     IntBuffer ColorBuffer, /* optional */
                     ShortBuffer IndexBuffer, int NrIndexes, Vec3f BoundMin, Vec3f BoundMax) {
