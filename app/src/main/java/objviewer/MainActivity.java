...
public class MainActivity extends Activity {
    ...
    float Rand ()
      /* returns a random float in [0.0 .. 1.0). */ {
        ...
        Random.nextBytes (V);
        return (float) (((int) V[0] & 255) | ((int) V[1] & 255) << 8 | ((int) V[2] & 255) << 16 | ((int) V[3] & 255) << 24) / 4294967296.0f;
    } /*Rand*/

    private ObjReader.Model ReadObj (final String ObjFileName) {
        ...
        try {
            Result=ObjReader.ReadObj (
                ...
                /*LoadMaterials =*/
                    new ObjReader.MaterialLoader () {
                        public ObjReader.MaterialSet Load (ObjReader.MaterialSet Materials, String MatFileName) {
                            return ObjReader.ReadMaterials (
                                    /*FileName =*/
                                    new java.io.File (new java.io.File (ObjFileName).getParentFile (), MatFileName).getPath (),
            ...
            android.widget.Toast.makeText (
                /*context =*/ this,
                /*text =*/
                    String.format (GLUseful.StdLocale, getString (R.string.obj_load_fail), Failed.toString ()),
    ...
    @Override
    public boolean onCreateOptionsMenu (android.view.Menu TheMenu) {
        OptionsMenu=new java.util.HashMap<android.view.MenuItem, Runnable> ();
        OptionsMenu.put (TheMenu.add (R.string.pick_file), new Runnable () {
            public void run () {
                startActivityForResult (new android.content.Intent (android.content.Intent.ACTION_PICK).setClass (MainActivity.this, Picker.class).putExtra (Picker.ExtensionID, ".obj").putExtra (Picker.LookInID, new String[]{"Models", "Download",}), LoadObjectRequest);
            } /*run*/
        } /*Runnable*/);
        OptionsMenu.put (TheMenu.add (R.string.reset_view), new Runnable () {
            public void run () {
                TheObjectView.ResetOrientation (true);
            } /*run*/
        } /*Runnable*/);
        OptionsMenu.put (TheMenu.add (R.string.options_lighting), new Runnable () {
            public void run () {
                new OptionsDialog (
                        ...
                        /*InitialButtonID =*/ TheObjectView.GetUseLighting () ? 1 : 0).AddButton (getString (R.string.on), 1).AddButton (getString (R.string.off), 0).show ();
            } /*run*/
        } /*Runnable*/);
        OptionsMenu.put (TheMenu.add (R.string.options_orient_faces), new Runnable () {
            public void run () {
                new OptionsDialog (
                        /*ctx =*/ MainActivity.this,
                        ...
                        /*InitialButtonID =*/ TheObjectView.GetClockwiseFaces () ? 1 : 0).AddButton (getString (R.string.anticlockwise), 0).AddButton (getString (R.string.clockwise), 1).show ();
            } /*run*/
        } /*Runnable*/);
        OptionsMenu.put (TheMenu.add (R.string.random_view), new Runnable () {
            public void run () {
                final float X=Rand (), Y=Rand (), Z=Rand (), R=(float) Math.sqrt (X * X + Y * Y + Z * Z), Angle=Rand () * 360.0f;
                TheObjectView.SetOrientation (new Rotation (Angle, true, X / R, Y / R, Z / R), true);
            } /*run*/
        } /*Runnable*/);
        return true;
    } /*onCreateOptionsMenu*/
    ...
    void BuildActivityResultActions () {
        ...
        ActivityResultActions.put (LoadObjectRequest, new RequestResponseAction () {
            public void Run (int ResultCode, android.content.Intent Data) {
                  /* Unfortunately I can't send this Intent directly from the Picker
                    ...
                    of MainActivity being created. Which is why I do it here. */
                startActivity (new android.content.Intent (android.content.Intent.ACTION_VIEW, Data.getData ()).setClass (MainActivity.this, MainActivity.class).setFlags (android.content.Intent.FLAG_ACTIVITY_SINGLE_TOP));
            } /*Run*/
        } /*RequestResponseAction*/);
    } /*BuildActivityResultActions*/

    @Override
    public void onCreate (android.os.Bundle ToRestore) {
        ...
        if (ToRestore != null) {
          /* reload previously-viewed object */
            ...
            android.widget.Toast.makeText (
                ...
                /*duration =*/ android.widget.Toast.LENGTH_LONG).show ();
        } /*if*/
        onNewIntent (getIntent ());
    } /*onCreate*/

    @Override
    protected void onNewIntent (android.content.Intent TheIntent) {
    ...
    public Object onRetainNonConfigurationInstance ()
      /* optimization to avoid re-reading .obj file */ {
        return TheObjectView.GetObject ();
    } /*onRetainNonConfigurationInstance*/

    @Override
    public void onSaveInstanceState (android.os.Bundle ToRestore) {
    ...
    @Override
    public boolean onOptionsItemSelected (android.view.MenuItem TheItem) {
    ...
    private class OptionsDialog extends android.app.Dialog implements android.content.DialogInterface.OnDismissListener {
        private final android.content.Context ctx;
        private final String Title;
        private final SelectedIDAction Action;
        private final int InitialButtonID;
        private final java.util.ArrayList<ButtonDef> TheButtonDefs=new java.util.ArrayList<ButtonDef> ();
        ...
        public void onCreate (android.os.Bundle ToRestore) {
            ...
            final android.view.ViewGroup.LayoutParams ButtonLayout=new android.view.ViewGroup.LayoutParams (android.view.ViewGroup.LayoutParams.FILL_PARENT, android.view.ViewGroup.LayoutParams.WRAP_CONTENT);
            for (ButtonDef ThisButtonDef : TheButtonDefs) {
                final android.widget.RadioButton ThisButton=new android.widget.RadioButton (ctx);
        ...
        @Override
        public void onDismiss (android.content.DialogInterface TheDialog) {
        ...
        private class ButtonDef {
            ...
            public ButtonDef (String ButtonTitle, int ButtonID) {
