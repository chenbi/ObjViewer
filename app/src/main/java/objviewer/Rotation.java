...
public class Rotation implements android.os.Parcelable {
    ...
    public static final android.os.Parcelable.Creator<Rotation> CREATOR=
      /* restore state from a Parcel. */
            new android.os.Parcelable.Creator<Rotation> () {
                public Rotation createFromParcel (android.os.Parcel Post) {
                    final android.os.Bundle MyState=Post.readBundle ();
                    return new Rotation (MyState.getFloat ("c", Null.c), MyState.getFloat ("s", Null.s), MyState.getFloat ("x", Null.x), MyState.getFloat ("y", Null.y), MyState.getFloat ("z", Null.z));
                } /*createFromParcel*/

                public Rotation[] newArray (int NrElts) {
                    return new Rotation[NrElts];
                } /*newArray*/
            } /*Parcelable.Creator*/;
  /* actually quaternion is (c, s * x, s * y, s * z) */
    ...
    public Rotation (float Angle, boolean Degrees, /* false for radians */
    ...
    private Rotation (float c, float x, float y, float z)
    ...
    private Rotation (float c, float s, float x, float y, float z)
      /* internal-use constructor with directly-computed components. Note
        this does not compensate for accumulated rounding errors. */ {
        this.c=c;
        this.s=s;
    ...
    @Override
    public void writeToParcel (android.os.Parcel Post, int Flags)
    ...
    public Rotation inv ()
      /* returns rotation by the opposite angle around the same axis. Or alternatively,
        ...
        return new Rotation (c, -s, x, y, z);
    } /*inv*/

    public Rotation mul (Rotation that)
      /* returns composition with another rotation. */ {
        final float s2=this.s * that.s;
        return new Rotation (this.c * that.c - (this.x * that.x + this.y * that.y + this.z * that.z) * s2, (this.y * that.z - this.z * that.y) * s2 + this.c * that.x * that.s + that.c * this.x * this.s, (this.z * that.x - this.x * that.z) * s2 + this.c * that.y * that.s + that.c * this.y * this.s, (this.x * that.y - this.y * that.x) * s2 + this.c * that.z * that.s + that.c * this.z * this.s);
    } /*mul*/

    public Rotation div (Rotation that)
      /* returns the difference from another rotation. */ {
        return mul (that.inv ());
    } /*div*/

    public Rotation mul (float Frac /* can also be negative or > 1 */)
      /* returns the specified fraction of the rotation. */ {
        return new Rotation (GetAngle (false) * Frac, false, x, y, z);
    } /*mul*/

    public float GetAngle (boolean Degrees /* false for radians */)
      /* returns the rotation angle. */ {
        final double Theta=Math.atan2 (s, c);
        return 2 * (float) (Degrees ? Math.toDegrees (Theta) : Theta);
    } /*GetAngle*/
    ...
    public String toString () {
        return String.format (GLUseful.StdLocale, "Rotation(%e, %e, %e, %e, %e)", c, s, x, y, z);
    } /*toString*/
