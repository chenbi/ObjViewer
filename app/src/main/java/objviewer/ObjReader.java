...
import static objviewer.GLUseful.gl;


public class ObjReader {

    public static Model ReadObj (java.io.InputStream From, MaterialLoader LoadMaterials) throws DataFormatException {
        ...
        for (; ; ) {
            ...
            if (Op == null && Parse.EOF || Op == "usemtl") {
                if (Faces != null) {
                  /* finish last object */
                    ...
                    for (FaceVert[] Face : Faces) {
                        for (FaceVert Vert : Face) {
                            if (!FaceMap.containsKey (Vert)) {
                                FaceMap.put (Vert, Geom.Add (
                                        /*Vertex =*/ Vertices.get (Vert.VertIndex),
                                        /*Normal =*/
                                        Vert.NormalIndex != null ? Normals.get (Vert.NormalIndex) : null,
                                        /*TexCoord =*/
                                        Vert.TexCoordIndex != null ? TexCoords.get (Vert.TexCoordIndex) : null,
                                        /*VertexColor =*/ null));
                            } /*if*/
                        } /*for*/
                    ...
                    if (BoundMin != null) {
                        BoundMin=new Vec3f (Math.min (BoundMin.x, NewObj.BoundMin.x), Math.min (BoundMin.y, NewObj.BoundMin.y), Math.min (BoundMin.z, NewObj.BoundMin.z));
                    } else {
                        BoundMin=NewObj.BoundMin;
                    } /*if*/
                    if (BoundMax != null) {
                        BoundMax=new Vec3f (Math.max (BoundMax.x, NewObj.BoundMax.x), Math.max (BoundMax.y, NewObj.BoundMax.y), Math.max (BoundMax.z, NewObj.BoundMax.z));
                    } else {
                ...
                if (Op == "v") {
                    ...
                    for (; ; ) {
                        ...
                        if (VertStr == null) break;
                        Integer VertIndex=null, TexCoordIndex=null, NormalIndex=null;
                        int Which=0, ThisPos=0, LastPos;
                        for (; ; ) {
                            LastPos=ThisPos;
                            for (; ; ) {
                                if (ThisPos == VertStr.length ()) break;
                                if (VertStr.charAt (ThisPos) == '/') break;
                                ++ThisPos;
                            } /*for*/
                            if (ThisPos > LastPos) {
                                int ThisComponent=-1; /*sigh*/
                                try {
                                    ThisComponent=Integer.parseInt (VertStr.substring (LastPos, ThisPos));
                  ...
                  /* Blender only expects a single library file name, which might have embedded spaces */
                    ...
                    if (LoadedMaterials == null) {
                        Parse.Fail ("materials loader didn't return anything");
                    } /*if*/
                } else {
            ...
            Parse.EndLine ();
            if (Parse.EOF && Faces == null) break;
        } /*for*/
        return new Model (ModelComponents.toArray (new Model.Component[ModelComponents.size ()]), BoundMin, BoundMax);
    } /*ReadObj*/

    public static MaterialSet ReadMaterials (java.io.InputStream From, MaterialSet CurMaterials) {
        final ObjTokenizer Parse=new ObjTokenizer (From);
        String MaterialName=null;
        GeomBuilder.Color AmbientColor=null, DiffuseColor=null, SpecularColor=null;
        Float SpecularExponent=null, Dissolve=null;
        Integer Illum=null;
        for (; ; ) {
            String Op=Parse.NextSym (false);
            if (Op != null) {
            ...
            if (Op == null && Parse.EOF || Op == "newmtl") {
                if (MaterialName != null) {
                    if (!CurMaterials.Materials.containsKey (MaterialName)) /* let earlier entry take precedence */ {
                        CurMaterials.Materials.put (MaterialName, new Material (
                                /*Name =*/ MaterialName,
            ...
            Parse.EndLine ();
            if (Parse.EOF && MaterialName == null) break;
        } /*for*/
        return CurMaterials;
    } /*ReadMaterials*/

    public static Model ReadObj (String FileName, MaterialLoader LoadMaterials) throws DataFormatException {
        Model Result=null;
    ...
    public static MaterialSet ReadMaterials (String FileName, MaterialSet CurMaterials) {
    ...
    public interface MaterialLoader {
        ...
        public MaterialSet Load (MaterialSet PrevMaterials, /* can simply add to this or replace it */
                                 String FileName);
    ...
    public static class Material {
        ...
        public final GeomBuilder.Color AmbientColor, DiffuseColor, SpecularColor;
        ...
        public Material (String Name,
          /* all following parameters optional, alpha components ignored, taken from Dissolve */
                         GeomBuilder.Color AmbientColor, GeomBuilder.Color DiffuseColor, GeomBuilder.Color SpecularColor, Float SpecularExponent, Float Dissolve, Integer Illum) {
            this.Name=Name;
            final float Alpha=Dissolve != null ? Dissolve : 1.0f;
            this.AmbientColor=AmbientColor != null ? new GeomBuilder.Color (AmbientColor.r, AmbientColor.g, AmbientColor.b, Alpha) : new GeomBuilder.Color (1.0f, 1.0f, 1.0f, Alpha);
            this.DiffuseColor=DiffuseColor != null ? new GeomBuilder.Color (DiffuseColor.r, DiffuseColor.g, DiffuseColor.b, Alpha) : new GeomBuilder.Color (1.0f, 1.0f, 1.0f, Alpha);
            this.SpecularColor=SpecularColor != null ? new GeomBuilder.Color (SpecularColor.r, SpecularColor.g, SpecularColor.b, Alpha) : new GeomBuilder.Color (1.0f, 1.0f, 1.0f, Alpha);
        ...
        public void Apply ()
          ...
          /* Illum NYI */
            gl.glMaterialfv (
                ...
                /*pname =*/ gl.GL_AMBIENT,
                /*params =*/
                    new float[]{AmbientColor.r, AmbientColor.g, AmbientColor.b, AmbientColor.a,},
                /*offset =*/ 0);
            gl.glMaterialfv (
                /*face =*/ gl.GL_FRONT_AND_BACK,
                /*pname =*/ gl.GL_DIFFUSE,
                /*params =*/
                    new float[]{DiffuseColor.r, DiffuseColor.g, DiffuseColor.b, DiffuseColor.a,},
                /*offset =*/ 0);
            gl.glMaterialfv (
                /*face =*/ gl.GL_FRONT_AND_BACK,
                /*pname =*/ gl.GL_SPECULAR,
                /*params =*/
                    new float[]{SpecularColor.r, SpecularColor.g, SpecularColor.b, SpecularColor.a,},
                /*offset =*/ 0);
            gl.glMaterialf (
                /*face =*/ gl.GL_FRONT_AND_BACK,
    ...
    public static class MaterialSet
      /* a mapping from material names to material definitions */ {
        private final java.util.Map<String, Material> Materials=new java.util.HashMap<String, Material> ();
    ...
    private static class ObjTokenizer {
        ...
        public void Fail (String Msg) {
            throw new DataFormatException (String.format (GLUseful.StdLocale, "ObjReader error at line %d, col %d: %s", LineNr, ColNr, Msg));
        } /*Fail*/
        ...
        public Vec3f GetVec (int MinD, /* [1 .. 3] */
                             int MaxD /* [3 .. 4] */) {
            final String XStr=NextSym (true);
            final String YStr=NextSym (MinD >= 2);
            final String ZStr=NextSym (MinD >= 3);
            final String WStr=MaxD == 4 ? NextSym (MinD == 4) : null;
            /*final*/
            Vec3f Result=null; /*sigh*/
            try {
                final float Y=YStr != null ? Float.parseFloat (YStr) : 1.0f;
                final float Z=ZStr != null ? Float.parseFloat (ZStr) : 1.0f;
                final float W=WStr != null ? Float.parseFloat (WStr) : 1.0f;
                Result=new Vec3f (Float.parseFloat (XStr) / W, Y / W, Z / W);
            } catch (NumberFormatException BadNum) {
        ...
        public GeomBuilder.Color GetColor () {
          /* colours in .obj don't have individual alpha */
            ...
            /*final*/
            GeomBuilder.Color Result=null; /*sigh*/
            try {
                Result=new GeomBuilder.Color (Float.parseFloat (RStr), Float.parseFloat (GStr), Float.parseFloat (BStr), 1.0f);
            } catch (NumberFormatException BadNum) {
    ...
    private static class FaceVert {
        ...
        public FaceVert (Integer VertIndex, /* mandatory */
    ...
    public static class Model {
        ...
        private Model (Component[] Components, Vec3f BoundMin, Vec3f BoundMax) {
