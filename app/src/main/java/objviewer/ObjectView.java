...
public class ObjectView extends android.opengl.GLSurfaceView {
    ...
    private final android.view.GestureDetector FlingDetector=new android.view.GestureDetector (getContext (), new android.view.GestureDetector.SimpleOnGestureListener () {
        @Override
        public boolean onFling (MotionEvent DownEvent, MotionEvent UpEvent, float XVelocity, float YVelocity) {
            final double CurrentTime=Useful.GetTime ();
            final float InitialAttenuate=5.0f; /* attenuates initial speed */
            final float FinalAttenuate=2.0f; /* attenuates duration of spin */
            final float SpinDuration=(float) Math.hypot (XVelocity, YVelocity) / (float) Math.hypot (getWidth (), getHeight ()) / FinalAttenuate;
            System.err.printf ("ObjViewer.ObjectView: spin duration = %.3fs\n", SpinDuration); /* debug */
            final PointF LastMouse=new PointF (UpEvent.getX (), UpEvent.getY ());
            final PointF ThisMouse=new PointF (LastMouse.x + XVelocity * SpinDuration / InitialAttenuate, LastMouse.y + YVelocity * SpinDuration / InitialAttenuate);
    ...
    public android.os.Parcelable onSaveInstanceState () {
        ...
        return new BundledSavedState (super.onSaveInstanceState (), MyState);
    } /*onSaveInstanceState*/
    ...
    private Rotation CalculateNewRotation (PointF LastMouse, PointF ThisMouse, boolean Fling /* debug */) {
        final PointF MidPoint=new PointF (getWidth () / 2.0f, getHeight () / 2.0f);
        final float Radius=(float) Math.hypot (ThisMouse.x - MidPoint.x, ThisMouse.y - MidPoint.y);
        final float DeltaR=Radius - (float) Math.hypot (LastMouse.x - MidPoint.x, LastMouse.y - MidPoint.y);
        ...
        final float ZAngle=Radius / MidDiag >= 0.5f ? (float) (Math.atan2 (ThisMouse.y - MidPoint.y, ThisMouse.x - MidPoint.x) - Math.atan2 (LastMouse.y - MidPoint.y, LastMouse.x - MidPoint.x)) : /* disable Z-rotation too close to centre where it’s too hard to control */
                0.0f;
        final float RotationSin=DeltaR / MidDiag;
          /* scale rotation angle by assuming depth of
            axis is equal to radius of view */
        { /* debug */
            final double RotationAngle=Math.toDegrees (Math.abs (RotationSin) > 1.0f ?
                      /* can happen with fling gestures */
                    RotationSin * (float) Math.PI / 2.0f : (float) Math.asin (RotationSin));
        ...
        return new Rotation /* X+Y axis */ (Math.abs (RotationSin) > 1.0f ?
                      /* can happen with fling gestures */
                Math.max (Math.min (RotationSin * (float) Math.PI / 2.0f, MaxRot), -MaxRot) : (float) Math.asin (RotationSin), false, (ThisMouse.y - MidPoint.y) / Radius, (ThisMouse.x - MidPoint.x) / Radius, 0).mul (new Rotation (Math.max (Math.min (ZAngle, MaxRot), -MaxRot), false, 0, 0, -1) /* Z axis */).mul (CurRotation);
    ...
    public void ResetOrientation (boolean Animate) {
        SetOrientation (Rotation.Null, Animate);
    } /*ResetOrientation*/

    public void SetObject (ObjReader.Model NewObject) {
        TheObject=NewObject;
    ...
    public void SetClockwiseFaces (boolean ClockwiseFaces) {
        this.ClockwiseFaces=ClockwiseFaces;
        requestRender ();
    } /*SetClockwiseFaces*/

    public boolean GetClockwiseFaces () {
        return ClockwiseFaces;
    } /*GetClockwiseFaces*/
    ...
    public void SetOrientation (Rotation NewOrientation, boolean Animate) {
        SetOrientation (
            /*NewOrientation =*/ NewOrientation,
            /*AnimFunction =*/
                Animate ? new PolyAccelerationDecelerationInterpolator (3.0f) : null,
            /*AnimDuration =*/ 1.5f);
    } /*SetOrientation*/
            ...
            if (TheObject != null) {
                if (UseLighting) {
                    ...
                    gl.glLightModelfv (
                        ...
                        /*offset =*/ 0); /* so hopefully objects are never completely black */
                ...
                final float MaxDim=Math.max (Math.max (TheObject.BoundMax.x - TheObject.BoundMin.x, TheObject.BoundMax.y - TheObject.BoundMin.y), TheObject.BoundMax.z - TheObject.BoundMin.z);
                final float Scale=2.5f;
                gl.glTranslatef (0.0f, 0.0f, -2.5f);
                CurRotation.Apply ();
                gl.glScalef (Scale / MaxDim, Scale / MaxDim, Scale / MaxDim);
                gl.glTranslatef (-(TheObject.BoundMax.x + TheObject.BoundMin.x) / 2.0f, -(TheObject.BoundMax.y + TheObject.BoundMin.y) / 2.0f, -(TheObject.BoundMax.z + TheObject.BoundMin.z) / 2.0f);
    ...
    private class RotationAnimator implements Runnable {
        ...
        public RotationAnimator (android.view.animation.Interpolator AnimFunction, double Duration, Rotation EndRotation) {
            this.AnimFunction=AnimFunction;
            this.StartTime=Useful.GetTime ();
        ...
        public void run () {
            if (CurrentAnim == this) {
                final double CurrentTime=Useful.GetTime ();
                final float AnimAmt=AnimFunction.getInterpolation ((float) ((CurrentTime - StartTime) / (EndTime - StartTime)));
